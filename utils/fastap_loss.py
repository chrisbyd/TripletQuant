...
class FastAP(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input,descriptor, target, num_bins):
        """
        Args:
        ...
        dist2 = 2 - 2 * torch.mm(input, descriptor.t())
        ...
        # 5. save for backward
        ctx.save_for_backward(descriptor, target)
        ctx.Z     = Z
        ctx.Delta = Delta
        ctx.dist2 = dist2
        ctx.I_pos = I_pos
    ...
    def backward(ctx, grad_output):
        ...
        for l in range(L):
          ...
          #  d_AP_y = d_AP_y - torch.mm(descriptor.t(), (Ap+An))

        grad_input = -d_AP_x
       
        return None,grad_input.t(), None,None
...
class FastAPLoss(torch.nn.Module):
    ...
    def forward(self, batch, descriptor,labels):
        return FastAP.apply(batch ,descriptor, labels, self.num_bins)
